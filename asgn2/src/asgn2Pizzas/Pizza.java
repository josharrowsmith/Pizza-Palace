...
public abstract class Pizza  {
	protected LinkedList<PizzaTopping> toppings  = new LinkedList<PizzaTopping>();
	...
	public Pizza(int quantity, LocalTime orderTime, LocalTime deliveryTime, String type, double price) throws PizzaException{
		...
		if (price > 20) throw new PizzaException("Price is too high");
		if (price <= 1) throw new PizzaException("Price is too low");
		if (quantity < 1) throw new PizzaException("Minimum one pizza per order");
		if (quantity > 10) throw new PizzaException("Maximum tem pizzas per order");
		if (deliveryTime.getHour() - orderTime.getHour() > 1) throw new PizzaException("Deliver took too long, Pizza thrown out");
		if (orderTime.getHour() < 19) throw new PizzaException("Store opens at 7pm");
		if ((orderTime.getHour() > 22 && orderTime.getMinute() > 50) || orderTime.getHour() > 23) throw new PizzaException("Store closes at 11pm");
		if (deliveryTime.getHour() > 23) throw new PizzaException("Deliveries end at 11.59pm");
	...
	public final void calculateCostPerPizza(){
		...
		for (PizzaTopping topping : toppings) {
			this.costPizza += topping.getCost();
		}
	}
	...
	public final double getOrderCost(){
		costPizza = this.getCostPerPizza();
		quantity = getQuantity();
		return this.costPizza * this.quantity;
	}
